...
export async function submitMarketplaceSupplierInvoiceAction(
  ...
  try {
    ...
    errorMessage = error instanceof Error ? error.message : "Invoice submission failed.";
  }

  if (errorMessage) {
    redirect(`/app/marketplace/orders?invoiceError=${encodeURIComponent(errorMessage)}`);
  }

  redirect(`/app/marketplace/orders?invoiceSubmitted=${encodeURIComponent(invoiceNumber ?? "Invoice")}`);
}
