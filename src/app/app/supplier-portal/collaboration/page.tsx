      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          <p className="text-xs font-black uppercase tracking-[.22em] text-blue-700">
            B6.1 · Supplier Collaboration Network
          </p>
          <h1 className="mt-3 text-4xl font-black">
            Supplier Collaboration Operations
          </h1>
          <p className="mt-3 max-w-4xl leading-7 text-slate-600">
            Govern supplier invoice submissions, shipment updates
            and persistent buyer–supplier conversations alongside
            the existing onboarding, questionnaire and task portal.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.invoices.map((invoice) => (
                <tr key={invoice.id}>
                  ...
                  <td className="px-4 py-3">{invoice.purchaseOrderRef ?? "—"}</td>
                  <td className="px-4 py-3">{invoice.currencyCode} {Number(invoice.invoiceAmount).toLocaleString()}</td>
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {data.shipments.map((shipment) => (
                <tr key={shipment.id}>
                  <td className="px-4 py-3">{shipment.supplier?.tradingName ?? shipment.supplier?.legalName ?? shipment.supplierId}</td>
                  <td className="px-4 py-3 font-bold">{shipment.shipmentReference}</td>
                  <td className="px-4 py-3">{shipment.trackingNumber ?? "—"}</td>
                  <td className="px-4 py-3">{shipment.estimatedDeliveryAt?.toLocaleString() ?? "—"}</td>
            ...
            <article key={thread.id} className={card}>
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-black">{thread.subject}</h2>
                  <p className="mt-1 text-xs text-slate-500">
                    {thread.supplier?.tradingName ?? thread.supplier?.legalName ?? thread.supplierId}
                    {thread.contextReference ? ` · ${thread.contextType}: ${thread.contextReference}` : ""}
                  </p>
                </div>
                ...
                {thread.messages.map((message) => (
                  <div key={message.id}>
                    <p className="text-[11px] font-black uppercase text-slate-500">
                      {message.senderType} · {message.senderEmail ?? "Enorsis"}
                    </p>
              ...
              <form action={replySupplierConversationAction} className="mt-4 flex gap-2">
                <input type="hidden" name="threadId" value={thread.id} />
                <input className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm" name="body" placeholder="Reply to supplier" required />
